# Copyright 2019 The TensorFlow Authors, Pavel Yakubovskiy, Bjorn Barz. All Rights Reserved.
#
...
# Bjorn Barz ([@Callidior](https://github.com/Callidior))
...
def mb_conv_block(inputs, block_args, activation, drop_rate=None, prefix='', spatial_dropout=False):
...
def EfficientNet(width_coefficient,
    ...
    x = layers.Conv2D(round_filters(32, width_coefficient, depth_divisor), 3,
                      ...
                      use_bias=False,
                      kernel_initializer=CONV_KERNEL_INITIALIZER,
                      name=change_name + 'stem_conv')(x)
    x = layers.BatchNormalization(axis=bn_axis, name=change_name + 'stem_bn')(x)
    x = layers.Activation(activation, name=change_name + 'stem_activation')(x)
    ...
    for idx, block_args in enumerate(blocks_args):
        ...
        x = mb_conv_block(x, block_args,
                          activation=activation,
                          drop_rate=drop_rate,
                          prefix=change_name + 'block{}a_'.format(idx + 1))
        block_num += 1
        if block_args.num_repeat > 1:
            ...
            for bidx in xrange(block_args.num_repeat - 1):
                drop_rate = drop_connect_rate * float(block_num) / num_blocks_total
                block_prefix = change_name + 'block{}{}_'.format(
                    idx + 1,
    ...
    x = layers.Conv2D(round_filters(1280, width_coefficient, depth_divisor), 1,
                      ...
                      use_bias=False,
                      kernel_initializer=CONV_KERNEL_INITIALIZER,
                      name='top_conv' + change_name)(x)
    x = layers.BatchNormalization(axis=bn_axis, name='top_bn' + change_name)(x)
    x = layers.Activation(activation, name='top_activation' + change_name)(x)
    if include_top:
        x = layers.GlobalAveragePooling2D(name='avg_pool' + change_name)(x)
        if dropout_rate and dropout_rate > 0:
            x = layers.Dropout(dropout_rate, name='top_dropout' + change_name)(x)
        x = layers.Dense(classes,
                         activation='softmax',
                         kernel_initializer=DENSE_KERNEL_INITIALIZER,
                         name='probs' + change_name)(x)
    else:
        if pooling == 'avg':
            x = layers.GlobalAveragePooling2D(name='avg_pool' + change_name)(x)
        elif pooling == 'max':
            x = layers.GlobalMaxPooling2D(name='max_pool' + change_name)(x)
    ...
    if weights == 'imagenet':
        ...
        if not spatial_dropout:
            ...
            _tensor = layers.Input(shape=(input_tensor.shape[1].value, input_tensor.shape[2].value, input_tensor.shape[3].value))
            initial_model = EfficientNetB32(input_tensor=_tensor, default_resolution=img_input.shape[1], weights='imagenet',
                                            include_top=False, input_shape=(img_input.shape[1], img_input.shape[1], 3), change_name=change_name+'cp_')
            ...
            for id, layer in enumerate(initial_model.layers):
                ...
                if "transformation" in model.layers[count].name:
                    count += 1
                if model.layers[count].name[:-1] in layer.name:
                    print("Igual", layer.name, model.layers[count].name)

                    if len(layer.get_weights()) == len(model.layers[count].get_weights()):
                        try:
                            model.layers[count].set_weights(layer.get_weights())
                        except ValueError:
                            print("Error jump")
...
def EfficientNetB32(include_top=True,
                   weights='imagenet',
                   input_tensor=None,
                   input_shape=None,
                   pooling=None,
                   default_resolution=244,
                   classes=1000,
                   spatial_dropout=False,
                   dropout=0.2,
                   change_name='',
                   **kwargs):
    return EfficientNet(1.0, 1.0, default_resolution, dropout=dropout,
